# -*- coding: utf-8 -*-
"""
Created on Wed Oct 11 09:45:22 2017
...
class Hconstruct:
    ...
    def __init__(self, k=np.array([0,0,0]), a = 1.61, ca_ratio = np.sqrt(8/3.)#1.61, #a is in Angstrom
                 d1=np.array([1,-np.sqrt(3),0]), d2=np.array([1,np.sqrt(3),0]), 
                 d3=np.array([0,0,1]), d4=np.array([0,-1,1]),
                 d5=np.array([0,1,-1]), d6=np.array([0,-1,-1]),
                 d7=np.array([-1,0,1]), d8=np.array([1,0,-1]), d9=np.array([-1,0,-1]), 
                d10=np.array([-1,1,0]), d11=np.array([1,-1,0]), d12=np.array([-1,-1,0]), b_atom = np.array([1./3, 2./3, 1./2])
                Es=1, Ep=1, 
        ...
        self.d1 = (self.a/2.)*d1
        self.d2 = (self.a/2.)*d2
        self.d3 = (self.a*self.ca_ratio)*d3
...
#
    ...
    def band_structure_p(self, ki, kf, ax, reverse, n1, n2):
        ...
        for i in range(loops):
            ...
            #self.phasefactors(kr )
            self.M = (1/4.)*self.Hamiltonian_p(kr)
