...
class Hconstruct:
    ...
    def __init__(self, k=np.array([0,0,0]), a = 1., d = ((8*np.pi/(3*np.sqrt(2)))**(1./3)), ca_ratio = np.sqrt(8/3.),#1.61, #a is in Angstrom
...
def sband_script(con):
    ...
    K_3 = np.array([4*np.pi/(3*con.a), 0, 0])
    ...
    H_3 = (2*np.pi)*np.array([2/(3*con.a), 0,(1./(2*con.c))])
    ...
    con.band_structure_s(Gamma, K_3, ax1, False, 'Gamma', 'K')
    con.band_structure_s(K_3, M, ax2, False, 'K', 'M')
    con.band_structure_s(M, Gamma, ax3, False, 'M', 'Gamma')
    con.band_structure_s(Gamma, A, ax4, False, 'Gamma', 'A')
    con.band_structure_s(K_3, H_3, ax5, False, 'K', 'H')
    con.band_structure_s(H_3, L, ax6, False, 'H', 'L')
    con.band_structure_s(L, A, ax7, False, 'L', 'A')
    con.band_structure_s(A, H_3, ax8, False, 'A', 'H')
...
def pband_script(con):
    ...
    K_2 = (2*np.pi/con.a)*np.array([(1/3),(1/np.sqrt(3.)),0])
    K_3 = np.array([4*np.pi/(3*con.a), 0, 0])
    K_4 = (2*np.pi/con.a)*np.array([(1/np.sqrt(3.)),(1/3),0])
    M = (np.pi/con.a)*np.array([1,(-1/np.sqrt(3)),0])
    A = (np.pi/con.c)*np.array([0,0,1])
    H = (2*np.pi)*np.array([(2./(3*con.a)),0,(1./(2*con.c))])
    H_2 = (2*np.pi)*np.array([(1./(3*con.a)),(1/(con.a*np.sqrt(3.))),(1./(2*con.c))])
    H_3 = (2*np.pi)*np.array([2/(3*con.a), 0,(1./(2*con.c))])
    H_4 = (2*np.pi)*np.array([(1/np.sqrt(3.))/con.a, (1/3)/con.a, 1./(2*con.c)])
    L = (np.pi)*np.array([(1./(con.a)),(-1./(np.sqrt(3)*con.a)),(1./(con.c))])
    ...
    con.band_structure_p(Gamma, K_3, ax1, False, 'Gamma', 'K')
    con.band_structure_p(K_3, M, ax2, False, 'K', 'M')
    con.band_structure_p(M, Gamma, ax3, False, 'M', 'Gamma')
    con.band_structure_p(Gamma, A, ax4, False, 'Gamma', 'A')
    con.band_structure_p(K_3, H_3, ax5, False, 'K', 'H')
    con.band_structure_p(H_3, L, ax6, False, 'H', 'L')
    con.band_structure_p(L, A, ax7, False, 'L', 'A')
    con.band_structure_p(A, H_3, ax8, False, 'A', 'H')
...
def dband_script(con):
    ...
    con.band_structure_d(Gamma, K_3, ax1, False, 'Gamma', 'K')
    con.band_structure_d(K_3, M, ax2, False, 'K', 'M')
    con.band_structure_d(M, Gamma, ax3, False, 'M', 'Gamma')
    con.band_structure_d(Gamma, A, ax4, False, 'Gamma', 'A')
    con.band_structure_d(K_3, H_3, ax5, False, 'K', 'H')
    con.band_structure_d(H_3, L, ax6, False, 'H', 'L')
    con.band_structure_d(L, A, ax7, False, 'L', 'A')
    con.band_structure_d(A, H_3, ax8, False, 'A', 'H')
