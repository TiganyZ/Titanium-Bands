...
class Hconstruct:
    ...
    def __init__(self, k=np.array([0,0,0]), H = np.ones(5), a = 1.,#1.61, #a is in Angstrom
                 d1=np.array([0,1,1]), d2=np.array([1,1,0]), 
                 d3=np.array([0,1,-1]), d4=np.array([-1,1,1]), Es=1, Ep=1, 
    ...
    def Hamiltonian_p(self, kv):
        ...
        M = np.array([
                [self.Ep*self.g0,  self.Exy*self.g3,  self.Exy*self.g2 ], 
                [self.Exy*self.g3c,  self.Ep*self.g0,  self.Exy*self.g1 ], 
                [self.Exy*self.g2c,  self.Exy*self.g1c,  self.Ep*self.g0 ]
                    ])
        #Array of Hamiltonian matrix with energy values 
        return M
    ...
    def band_structure_p(self, ki, kf):
        ...
        fig = plt.figure()
        ax = fig.add_subplot(111)
        ax.plot(self.kvals, np.abs(self.px_energies))#, marker=style, color=k)
        ax.plot(self.kvals, np.abs(self.py_energies))#, bo)
        ax.plot(self.kvals, np.abs(self.pz_energies))#, r*)
        ax.set_xlabel('k')
        ax.set_ylabel('E')
