    ...
    def initial_energies(self, signifier, band):
        ...
        if signifier == 'fcc':
            ...
            if band == 'd':
                ...
                self.Edxxyy_zr = ((1./2)*np.sqrt(3)*(ll - mm)*(nn - 0.5*(ll + mm))*self.ddsig + np.sqrt(3)*nn*(mm - ll)*self.ddpi + 0.25*np.sqrt(3)*(ones + nn)*(ll -  mm)*self.dddelta)
                self.Edzr_zr = ((nn - 0.5*(ll + mm))*self.ddsig + 3*nn*(mm + ll)*self.ddpi + 0.75*((ll +  mm)**2)*self.dddelta)
