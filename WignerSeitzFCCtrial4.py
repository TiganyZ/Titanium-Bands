    ...
    def initial_energies(self, signifier, band):
        ...
        if signifier == 'fcc':
            ...
            if band == 'd':
                ...
                self.Edxy_xy = ( 3*self.ll*self.mm*self.ddsig     +   (self.ll + self.mm -4*self.ll*self.mm)*self.ddpi    +    (self.nn + self.ll*self.mm)*self.dddelta )
                self.Edyz_yz = (3*self.mm*self.nn*self.ddsig      +   (self.mm + self.nn -4*self.mm*self.nn)*self.ddpi    +    (self.ll + self.mm*self.nn)*self.dddelta)
                self.Edzx_zx = (3*self.nn*self.ll*self.ddsig      +   (self.nn + self.ll -4*self.nn*self.ll)*self.ddpi    +    (self.mm + self.nn*self.ll)*self.dddelta)
                
                self.Edxy_yz = (3*self.l*self.mm*self.n*self.ddsig     +   self.l*self.n*(ones -  4*self.mm )*self.ddpi   +    self.l*self.n*(self.mm - ones)*self.dddelta)
                self.Edxy_zx = (3*self.ll*self.m*self.n*self.ddsig     +   self.m*self.n*(ones -  4*self.ll )*self.ddpi   +    self.m*self.n*(self.ll - ones)*self.dddelta)
                self.Edyz_zx = (3*self.nn*self.m*self.l*self.ddsig     +   self.m*self.l*(ones -  4*self.nn )*self.ddpi   +    self.m*self.l*(nn - ones)*self.dddelta)
                
                self.Edxy_xxyy = ((3./2)*self.l*self.m*(self.ll - self.mm)*self.ddsig       +       2*self.l*self.m*(self.mm -  self.ll )*self.ddpi                 +    0.5*self.l*self.m*(self.ll - self.mm)*self.dddelta)
                self.Edyz_xxyy = ((3./2)*self.m*self.n*(self.ll - self.mm)*self.ddsig       -       self.m*self.n*(ones + 2*(self.ll -  self.mm) )*self.ddpi        +    self.m*self.n*(ones + 0.5*(self.ll - self.mm))*self.dddelta)
                self.Edzx_xxyy = ((3./2)*self.n*self.l*(self.ll - self.mm)*self.ddsig       +       self.n*self.l*(ones - 2*(self.ll -  self.mm) )*self.ddpi        -    self.n*self.l*(ones - 0.5*(self.ll - self.mm))*self.dddelta)
                
                self.Edxy_zr = (np.sqrt(3)*self.l*self.m*(self.nn - 0.5*(self.ll + self.mm))*self.ddsig      -   np.sqrt(3)*2*self.l*self.m*self.nn*self.ddpi               +   0.5*np.sqrt(3)*self.l*self.m*(ones + self.nn)*self.dddelta)
                self.Edyz_zr = (np.sqrt(3)*self.m*self.n*(self.nn - 0.5*(self.ll + self.mm))*self.ddsig      +   np.sqrt(3)*self.m*self.n*(self.ll + self.mm - self.nn)*self.ddpi     -   0.5*np.sqrt(3)*self.m*self.n*(self.ll + self.nn)*self.dddelta)
                self.Edzx_zr = (np.sqrt(3)*self.l*self.n*(self.nn - 0.5*(self.ll + self.mm))*self.ddsig      +   np.sqrt(3)*self.l*self.n*(self.ll + self.mm - self.nn)*self.ddpi     -   0.5*np.sqrt(3)*self.l*self.n*(self.ll + self.nn)*self.dddelta)
                
                self.Edxxyy_xxyy = (0.75*((self.ll - self.mm)**2)*self.ddsig      +     (self.ll + self.mm - (self.ll -  self.mm)**2 )*self.ddpi    +    (self.nn + 0.25*(self.ll -  self.mm)**2)*self.dddelta)
                self.Edxxyy_zr = (0.5*np.sqrt(3)*(self.ll - self.mm)*(self.nn - 0.5*(self.ll + self.mm))*self.ddsig     +   np.sqrt(3)*self.nn*(self.mm - self.ll)*self.ddpi     +   0.25*np.sqrt(3)*(ones + self.nn)*(self.ll -  self.mm)*self.dddelta)
                self.Edzr_zr = (((self.nn - 0.5*(self.ll + self.mm))**2)*self.ddsig       +      3*self.nn*(self.mm + self.ll)*self.ddpi      +       0.75*((self.ll +  self.mm)**2)*self.dddelta)
